    ...
    UserModel.findOne({ _id: req.params.id }) /*Otherwise, it searches the database and selects the User object corresponding to the identifier passed as a parameter*/
        .then(user => {
            ...
            const userObject = req.file ? {
                ...req.body,
                picture: `https://julienjamet-groupomania.com/uploads/profil/${req.file.filename}`,
                id: user._id,
            ...
            if (req.file !== undefined && filename !== "random-user.png") {
                fs.unlink(`https://julienjamet-groupomania.com/public/uploads/profil/${filename}`, () => {
    ...
    UserModel.findOne({ _id: req.params.id }) /*Otherwise, it searches the database and selects the User object corresponding to the identifier passed as a parameter...*/
        .then(user => {
            ...
            if (filename !== "random-user.png") {
                fs.unlink(`https://julienjamet-groupomania.com/public/uploads/profil/${filename}`, () => { /*...then runs the FS unlink() function to delete this image from the folder...*/
